...
module.exports = new Command({
  ...
  async run(interaction) {
    ...
    collector.on('collect', async (i) => {
      ...
      // Fix 6: re-proveri permisiju na svaki collect (fresh fetch, ne cached)
      try {
        const freshMember = await interaction.guild.members.fetch(i.user.id);
        const hasStaff = freshMember.roles.cache.has(SupportStaff.id);
        const hasAdmin = freshMember.roles.cache.has(Admin.id);
        if (!hasStaff && !hasAdmin) {
          await i.reply({
            content: 'You no longer have permission to edit this message. Stopping editor.',
            flags: MessageFlags.Ephemeral
          });
          collector.stop();
          return;
        ...
        // member ne postoji više ili fetch fail — zaustavi
        collector.stop();
        return;
      }
